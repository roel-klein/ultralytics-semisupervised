...
class ExclusiveLoss:  # pylint: disable=R0902
    ...
    def __init__(  # pylint: disable=R0913
        ...
        self.epoch = epoch
        self.unannotated_mapping = self.last_dim_allzero_as_unannotated
    ...
    @staticmethod
    def last_dim_allzero_as_unannotated(targets : Tensor) -> Tensor:
        return targets.sum(axis=-1) == 0
    ...
    def forward_(
        ...
        """
        # TODO our input is now shape (BATCHxBOXESxCLASSES)
        # this we can melt this into (COMBINEDBOXES x CLASSES), and leave 
        ...
        unannotated_loss = self.focal_loss(cross_entropy) * background_sampling[..., None]
        annotated_loss = cross_entropy * (~unannotated_mask[..., None] * 1.0)
...
class BECEWithLogitsLossMulticlass(ExclusiveLoss, BCEWithLogitsLoss):
    """
    Binary Exclusive cross-entropy with sigmoid activation loss as described in the Sparse-shot Learning with
    Exclusive Cross-Entropy for Extremely Many Localisations paper[1].
    It is a subclass of the abstract class ExclusiveLoss.
